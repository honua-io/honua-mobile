...
describe('display adapter', () => {
  ...
  it('does not match object id deletes against unrelated feature ids', () => {
    const initial = featureQueryResultToGeoJson({
      source: fieldAssetSource,
      objectIdFieldName: 'objectid',
      features: [
    ...
    expect(deleted.features.map((feature) => feature.properties.name)).toEqual(['Feature id 1']);
  });
