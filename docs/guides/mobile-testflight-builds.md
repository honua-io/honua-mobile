...
The workflow uses `macos-26` and explicitly selects Xcode 26.4 from
`/Applications/Xcode_26.4.app`. The current .NET 10 iOS workload
`Microsoft.iOS.Sdk.net10.0_26.4` requires Xcode 26.4. Builds fail early with an
actionable error if a runner only has Xcode 26.3.

When the Microsoft iOS workload moves to a newer Xcode requirement, update the
workflow `REQUIRED_XCODE_VERSION` and runner image together.
