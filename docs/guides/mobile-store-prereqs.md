...
The Android internal distribution and store preflight workflows read one app's
signing material from the selected protected environment. Values must be set
only in protected environments, not as repository-wide secrets.

- `GOOGLE_PLAY_SERVICE_ACCOUNT_JSON`
  Play Console service account JSON with app-scoped permissions.
- `ANDROID_PACKAGE_NAME`
  Package name for the selected app. This must match the selected MAUI
  project's `<ApplicationId>`.
- `ANDROID_KEYSTORE_BASE64`
  Base64-encoded upload keystore file for the selected package ID.
- `ANDROID_KEYSTORE_PASSWORD`
  Upload keystore password for the selected package ID.
- `ANDROID_KEY_ALIAS`
  Upload key alias for the selected package ID.
- `ANDROID_KEY_PASSWORD`
  Upload key password for the selected package ID.

Use separate protected environments when apps have different upload keys. For
example, `android-internal-fieldcollection` can hold `ANDROID_PACKAGE_NAME` =
`io.honua.mobile.fieldcollection`, while `android-internal-app` can hold
`ANDROID_PACKAGE_NAME` = `io.honua.mobile.app`.

The secure vault may use app-specific inventory names, but the GitHub
environment exposes the workflow-facing names above. Run **Android Store
Preflight** after wiring or rotating these secrets.
