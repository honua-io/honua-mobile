...
GeoPackage storage runs through `GeoPackageSyncStore` and is configured via
`GeoPackageSyncStoreOptions`. Cleanup of cached feature rows is driven by the
per-layer TTL policy shown above plus GeoPackage compaction; there is no
separate `Cleanup` / `ClearOfflineData` API on `HonuaMobileClient`.

Sync progress is observed through `IOfflineSyncRunner` results and the
telemetry counters listed in
[Sync Problems And Telemetry](#sync-problems-and-telemetry).

For end-to-end offline test patterns see
[`tests/Honua.Mobile.Offline.Tests`](../../tests/Honua.Mobile.Offline.Tests) and
[`tests/Honua.Mobile.FieldCollection.Tests`](../../tests/Honua.Mobile.FieldCollection.Tests),
which exercise the real `OfflineSyncEngine`, queue claim/lease semantics, and
